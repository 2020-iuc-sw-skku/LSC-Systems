...
@param np.array x: 웨이퍼 맵
@param int num: 웨이퍼 맵을 분할하는 수 
@return: 구역별로 추출된 밀도 특성 
@rtype: pd.DataFrame
...
def extract_density(x, num=6):
    ...
    max_digit = int(log10((num-2)**2+4))+1
    # 해당 영역에서의 밀도를 구한다 
    calculate_density = lambda x: 100*(np.sum(x==2)/np.sum(np.logical_or(x==1, x==2)))
    # 특성명을 생성한다. 남는 자릿수는 0으로 패딩한다 (특성명을 사전순으로 배열할때 순서가 깨지는 것을 방지하기 위함)
    feature_name = lambda x: f"density_{str(x).zfill(max_digit)}"
    ...
    for i in range(1, num-1):
        for j in range(1, num-1):
            region = x[interval_row[i]:interval_row[i+1], interval_col[j]:interval_col[j+1]]
            density[feature_name(count)] = calculate_density(region)
            count += 1

    # 바깥 부분의 밀도
    density[feature_name(count)] = calculate_density(x[interval_row[0]:interval_row[1], :])
    count += 1
    density[feature_name(count)] = calculate_density(x[:, interval_col[num-1]:])
    count += 1
    density[feature_name(count)] = calculate_density(x[interval_row[num-1]:, :])
    count += 1
    density[feature_name(count)] = calculate_density(x[:, interval_col[0]:interval_col[1]])
